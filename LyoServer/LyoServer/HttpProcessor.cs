...
namespace LyoServer
{
    public class HttpProcessor : MarshalByRefObject
    {
        ...
        public void ProcessRequest()
        {
            try
            {
                ...
                if (requestInfo != null)
                {
                    ...
                    {
                        WriteFileResponse(requestInfo, staticContentType);
                    }
                    //检查是否为目录
                    else if (requestInfo.FilePath.EndsWith("/"))
                    {
                        WriteDirResponse(requestInfo);
                    }
                    //动态网页处理方法
                    else
                    {
        ...
        private void WriteFileResponse(RequestInfo req, string contentType)
        {
            string fullPath = Path.Combine(_host.PhysicalDir, req.FilePath.TrimStart('/'));
            if (!File.Exists(fullPath))
                SendErrorResponse(404);
            else
            {
                Console.ForegroundColor = ConsoleColor.Green;
                Console.WriteLine(string.Format("{0} {1} from {2}", contentType, req.RawUrl, req.RemoteEndPoint));
                try
                {
        ...
        /// </summary>
        /// <param name="req"></param>
        private void WriteDirResponse(RequestInfo req)
        {
            string dir = Path.Combine(_host.PhysicalDir, req.FilePath.TrimStart('/'));
            if (!Directory.Exists(dir))
                SendErrorResponse(404);
            else
            {
                Console.ForegroundColor = ConsoleColor.DarkRed;
                Console.WriteLine(string.Format("Request to list path of {0} from {1}", dir, req.RemoteEndPoint));
                Console.WriteLine("Request to list path:" + dir);
